...
**Compartment title:** Stream.

## The three

| ID | File | Middle state |
|----|------|----------------|
| **ST1** | [`plates/travis-st1-in-this-room-stream.png`](./plates/travis-st1-in-this-room-stream.png) | In this room. One seat glowing. That mark is the door. |
| **ST2** | [`plates/travis-st2-stream.png`](./plates/travis-st2-stream.png) | Stream. Trigger pinned. Live message + Term still emitting. |
| **ST3** | [`plates/travis-st3-log-stream-card.png`](./plates/travis-st3-log-stream-card.png) | Log. Done stream is a card **above** the completed line. |

One plate per hole. Not three frames of the same idea.

## Do not build from the PNG
...
| Bucket | Draft |
|--------|--------|
| **1. Copy** | In this room → glowing seat → Stream. Trigger stays top. Live order in the compartment. Done → card on the Log above the completed message. Click card = same contents as Stream. |
| **2. Do not build** | Handle bars, loud Add, labeled Term if we recut, fake diffs, bezel. |
