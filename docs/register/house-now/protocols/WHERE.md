...
| Engineer | `docs/seats/ENGINEER.md` + repo-root `AGENTS.md` | git + PRs. **One PR per initiative.** `README.md` Implementation line. `ENGINEER-HANDOFF.md`. `initiatives/<nnn>-<slug>/`. `HOTFIX-NNN-*.md` + `HOTFIXES.md`. Work in `src/`. | PM log. SA log. |
...
Create is a person. Seated is when a person is hung on one file in `/protocols`. Sit is planted (015). This file is still the map.
