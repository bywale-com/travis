...
**Current (read first, then the newest stamp at the bottom):** 2026-08-29 15:44 UTC — **SCP-006 cut.** Travis is a fourth `agent_binding` row (no Cursor id). Live when dest is Travis. Seat pipe unchanged. Handoff is go.
...
## 2026-08-29 15:44 UTC — Pass on Engineer envelope (Travis agent)

**Why:** Founder: work the new packet from the Engineer so Engineer can pick it up. Wrap: [`ENVELOPE-TRAVIS-ROOM-AGENT.md`](https://github.com/bywale-com/travis/pull/29). Not a PM plate pass. Did not plant.

**Locks (Engineer asked; SA decided — no leftover):**

1. Addressee = **fourth binding** `seat_key=travis`, empty `cursor_agent_id`, never `resume`. Not a new session field.  
2. Travis speech = `agent_post` + `seat_key=travis`. Dead-man stays `travis_prompt`.  
3. Live: `GEMINI_API_KEY` server-only; ephemeral token to phone; `voice_session.travis_live_handle` for resume. Model id in runtime, not a table.  
4. Travis is **not** queued. Barge-in. `seat_live_run` is Cursor-only.  
5. Dest Travis: no “I’m done” conductor. Seats still use the phrase.  
6. Type mixed `@` Travis + a seat: both fire; sticky = last chip.  
7. Refuse: `Agent.create`, agent catalog, ElevenLabs, extra rooms, Live mouth for other seats.

**Cut:** [`SYSTEMS-CHANGE-PACKET-006-TRAVIS-AGENT.md`](./SYSTEMS-CHANGE-PACKET-006-TRAVIS-AGENT.md)

**Handoff is go.** Engineer plants.
